...
function AuthProvider(props) {
    ...
    const handleLogin = async (data) => {
        try {
            console.log("From handle Login ", data.user);
            //STORE DATA
            let { token, user } = data;
            let data_ = [[USER_KEY, JSON.stringify(data.user)], [TOKEN_KEY, data.user.token]];
            await AsyncStorage.multiSet(data_);
            // await AsyncStorage.setItem(TOKEN_KEY, data.user.token);
